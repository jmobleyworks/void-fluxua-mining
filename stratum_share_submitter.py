#!/usr/bin/env python3
"""
Stratum Share Submitter - Sends real submissions to MoneroOcean pool
Reads from /tmp/stratum_submissions.jsonl and submits shares to the actual pool
"""
...
LOG_FILE = "/tmp/stratum_share_submitter.log"
SUBMISSION_FILE = "/tmp/stratum_submissions.jsonl"
...
def connect_to_pool():
    ...
    try:
        ...
        log_msg(f"📤 Sent login for {WALLET[:20]}...")

        # Try to get initial job
        try:
            initial_data = socket_conn.recv(4096).decode()
            if initial_data:
                log_msg(f"📥 Pool response: {initial_data[:100]}")
                # Try to parse for job_id
                try:
                ...
                for line in response_data.split('\n'):
                    if line.strip():
                        try:
                            ...
                            if resp.get('error'):
                                stats['rejected'] += 1
                                if stats['rejected'] % 50 == 0:
                                    log_msg(f"⚠️ Share rejected: {resp.get('error')}")
                            elif 'result' in resp:
                                stats['accepted'] += 1
                                if stats['accepted'] % 10 == 0:
                                    log_msg(f"✅ Share ACCEPTED #{stats['accepted']}")
                        except:
                            pass
...
def process_submissions():
    """Read and submit shares from submission file"""
...
def main():
    """Main loop"""
    log_msg("Stratum Share Submitter started (using LOCAL PROXY at 127.0.0.1:9999)")
